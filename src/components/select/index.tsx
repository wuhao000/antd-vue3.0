...
import {defineComponent, getCurrentInstance, ref, App} from 'vue';
import {filterEmpty, getComponentFromProp, getListeners, getOptionProps, isValidElement} from '../_util/props-util';
...
import {OptGroup, Option, Select as VcSelect} from '../vc-select';
...
const Select = defineComponent({
  ...
  setup(props) {
    ...
    const getNotFoundContent = (renderEmpty) => {
      const notFoundContent = getComponentFromProp(instance, 'notFoundContent');
    ...
    const renderSuffixIcon = (prefixCls) => {
      const {loading} = props;
    ...
    return {getNotFoundContent, configProvider, isCombobox, renderSuffixIcon, blur, focus, savePopupRef};
  },
  render(ctx) {
    const {
    ...
    const renderEmpty = ctx.configProvider.renderEmpty;
    const prefixCls = getPrefixCls('select', customizePrefixCls);
    const instance = getCurrentInstance();

    const {getPopupContainer: getContextPopupContainer} = this.configProvider;
    let removeIcon = getComponentFromProp(instance, 'removeIcon');
    removeIcon = Array.isArray(removeIcon) ? removeIcon[0] : removeIcon;
    let clearIcon = getComponentFromProp(instance, 'clearIcon');
    clearIcon = Array.isArray(clearIcon) ? clearIcon[0] : clearIcon;
    let menuItemSelectedIcon = getComponentFromProp(instance, 'menuItemSelectedIcon');
    ...
    const selectProps = {
      ...
      maxTagPlaceholder: getComponentFromProp(instance, 'maxTagPlaceholder'),
      placeholder: getComponentFromProp(instance, 'placeholder'),
      children: options
        ? options.map(option => {
      ...
      dropdownRender: getComponentFromProp(getCurrentInstance(), 'dropdownRender', {}, false),
